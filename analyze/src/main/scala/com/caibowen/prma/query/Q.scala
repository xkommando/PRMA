...
object Q {
  ...
  val _logggerNameByID = new SQLOperation("SELECT `value` FROM `logger` WHERE id=?")
  ...
  val _threadNameByID = new SQLOperation("SELECT `value` FROM `thread` WHERE id=?")
  ...
  val _tagsByEventID = new SQLOperation("SELECT TG.value FROM `tag` as TG INNER JOIN `j_event_tag` AS JET ON JET.tag_id = TG.id WHERE JET.event_id = ?")
  def tagsByEventID(id: Long)(implicit session: DBSession): Set[String] =
    _tagsByEventID.vector(colStr, _.setLong(1, id))(session).toSet

  val _propsByEventID = new SQLOperation("SELECT PROP.key, PROP.value FROM `property` as PROP INNER JOIN `j_event_prop` AS JEP ON JEP.prop_id = PROP.id WHERE JEP.event_id = ?")
  ...
  val _stackTraceByID = new SQLOperation("SELECT SCK.file, SCK.class, SCK.function, SCK.line FROM 'stack_trace' as SCK WHERE SCK.id = ?")
  //  String declaringClass, String methodName,
  //  String fileName, int lineNumber) {
  ...
  val _exceptStackTraces = new SQLOperation("SELECT SCK.file, SCK.class, SCK.function, SCK.line FROM 'stack_trace' as SCK INNER JOIN `j_exception_stacktrace` AS JES on JES.stacktrace_id = SCK.id WHERE JES.except_id = ?")
  ...
  val _exceptByEventID = new SQLOperation("SELECT EXP.id, EXP.name, EXP.msg FROM `exception` AS EXP INNER JOIN j_event_exception AS JEE ON JEE.except_id = EXP.id WHERE JEE.event_id = ?")
